...
| `GET /v1/ct/{token}/account/{address}` | The PUBLIC on-chain ConfidentialAccount record (registered viewing key + balance commitments), proxied for client-side verification |
| `GET /` | The product page: in-browser statement reader (same crypto as `umbra view`, compiled to WASM) — keys never leave the page, results verified against on-chain commitments. Operational dashboard at `GET /status` |
